...
```{r}
fare_settings$fare_per_mode
```

We need to do two small changes in the `fare_per_mode` table, specifically in the `"RAIL"` mode: setting `unlimited_transfers` to `TRUE`, and updating fare to 450. The `"BUS"` mode does not need any change, so we're leaving it with the default values. We'll do those changes below, using data.table notation.

```{r}
fare_settings$fare_per_mode[mode == "RAIL", unlimited_transfers := TRUE]
fare_settings$fare_per_mode[mode == "RAIL", fare := 450]
```
...
```{r}
fare_settings$fare_per_transfer
```

Let's update `fare_per_transfer` to account for the actual integration rules in Porto Alegre. 

- The fare for "BUS" to "BUS" integration is composed of 480 for the first leg plus 240 for the second leg, which equals to a total fare of 720. Also, there is a special rule that the discount is not valid when transferring between buses within the same route (for example, from bus T1 to another T1), so we need to set `allow_same_route` to `FALSE`. 

```{r}
fare_settings$fare_per_transfer[leg1 == "BUS" & leg2 == "BUS", 
                                allow_same_route := FALSE]
...
- Transfers between "RAIL" and "RAIL" are free and unlimited, which is already accounted for in the field `unlimited_transfers` of the `fare_per_mode` table. Thus, the equivalent line of the `fare_per_transfer` data.frame can be completely removed. 

```{r}
fare_settings$fare_per_transfer <- 
  fare_settings$fare_per_transfer[!(leg1 == "RAIL" & leg2 == "RAIL")]
```
...
```{r}
fare_settings$fare_per_transfer[leg1 != leg2, fare := 837]
```
...
```{r}
tail(fare_settings$routes_info)
```
...
```{r}
r5r_core$setFareCalculatorDebugOutput(
  here::here("tests_marcus", "fare_calculator_output.csv"))
```


```{r}
access_f <- accessibility(r5r_core,
...
```{r}
debug_output <- fread(here::here("tests_marcus", "fare_calculator_output.csv"))

debug_output[, count := stringr::str_count(debug_output$pattern, 
                                           pattern = "\\|") + 1]
```

```{r}
debug_output[stringr::str_starts(debug_output$pattern, "LINHA")] |> View()
```

```{r}
View(fare_settings$fare_per_transfer)
```
