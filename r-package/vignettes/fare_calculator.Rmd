...
abstract: "This vignette shows how to configure and use custom fare calculators in order to account for monetary travel costs when generating travel time matrices and accessibility estimates with the `r5r` package."
urlcolor: blue
vignette: >
...
First, we need to call `setup_fare_calculator()`, providing three parameters: the current `r5r_core` object, a `base_fare` used to populate the fare calculator, and the `by` parameters that identifies what is the main property of the route that defines the different fares.
...
Below, we can see a sample of the travel time differences with and without monetary cost restriction. We can see that some trips are not affected at all (`travel_time_unl == travel_time_500`), some trips take a little longer to complete (`travel_time_500 > travel_time_unl`), and other trips cannot be completed at all (`travel_time_500 == NA`).
