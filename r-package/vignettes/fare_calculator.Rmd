...
-   Each train ticket costs R\$ 4.50. Once the user enters a train station, they can take an unlimited amount of trains as long as they do not leave a station.
...
Let's configure the global properties first, which are the ones that are applied to the entire system. First, we can see that `base_fare` has already been configured to 4.80, so we leave it as that. `max_discounted_transfers` is set to 1 by default, meaning that the traveler can get 1 integrated discount when riding multiple buses, but after that tickets are charged at full price. `transfer_time_allowance` is set to 120 minutes by default, so we have to set it to 60 minutes to fit our use case (travelers have 60 minutes to take the second bus on a discounted fare, otherwise a full fare is charged). Finally, the `fare_cap` setting indicates if there is a maximum value that can be charged in a trip, beyond which all subsequent rides are free of charge. In this example, we can leave `fare_cap` set to its default `Inf` value because this feature is not available in Porto Alegre.
...
To configure mode-, transfer-, and route-specific properties, we can use the three data.frames inside `fare_settings`. Let's configure the modes first. Below, we can see that the `fares_per_mode` data.frame contains five columns:
...
-   `allow_same_route_transfer`: a boolean indicating if a discounted transfer can be done betwen buses of the same route;
...
Transfer fares settings are stored in the `fares_per_transfer` data.frame, which is shown below. Each row contains ticket prices for transfers between the modes specified in `first_leg` and `second_leg` columns.
...
Information on specific routes is stored in the `fares_per_route` data.frame. Below, we can see a sample of the bus and train routes in Porto Alegre.
...
ggplot(data = access) +
  geom_sf(aes(fill = accessibility), size = 0.2) +
