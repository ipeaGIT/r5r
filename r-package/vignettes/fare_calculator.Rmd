...
fare calculators in routing models, making it possible to estimate travel time 
...
that a specific fare calculator for each city needs to be programmed in Java and 
...
calculator that can be configured via a predefined set of properties and rules 
...
**This vignette shows the features of `r5r`s fare calculator. It also uses a 
reproducible example to demonstrate how to configure the fare calculator to 
...
number of discounted transfers the user can make and/or a time limit for using that discount. This is the type of fare rule currently covered by `r5r`.
...
Alegre (Brazil) included in `r5r`. Porto Alegre has a relatively straightforward 
public transport system, where the vast majority of the population that rely on 
transit ride buses. The city also has a metropolitan rail service that connects 
the city center to the neighboring northbound municipalities. That system can be 
...
current `r5r_core` object, a `base_fare` used to populate the fare calculator, and
...
fares and integration rules. Users can also create a fare calculator where fare 
...
To configure mode-, transfer-, and route-specific properties, we can use the three `data.frames` inside our `fare_settings` list. Let's configure the modes first. 
Below, we can see that the `fares_per_mode` data.frame contains five columns:
...
We need to do a few small changes in the `fares_per_mode` table to accomodate the
...
specifically for the `r5r` fare calculator.
...
column in the `fares_per_mode` table: the `route_fare` field is only considered 
by the `r5r` fare calculator when `use_route_fare` of that mode is set to `TRUE`.

-   `fare_type`: is used to link each route with information in the `fares_per_mode` 
and `fares_per_transfer` tables. In this example, `fare_type` is always the same 
as `mode`, because that was what we chose in the `by` parameter when calling `setup_fare_calculator` earlier (we could have chosen to discriminate fares by 
agency, for example).
...
lines, because we are using the fares set in `fares_per_mode` and `fares_per_transfer` 
which we already set up correctly before.

Now that our `fare_settings` is complete, we can use it to calculate travel time 
...
access_500 <- calculate_accessibility(5, "R$ 5.00 budget")
access_unl <- calculate_accessibility(Inf, "Unlimited budget")
