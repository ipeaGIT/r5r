---
title: 'Build a custom network'
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
abstract: "This vignette shows how to build a custom transport network with modified car speeds, which can be used to simulate different scenarios of traffic congestion and road closures."
urlcolor: blue
vignette: >
  %\VignetteIndexEntry{Build a custom network} 
...
The `build_custom_network()` has a relatively similar behavior to the `r5r::setup_r5()` function. It builds a transport network that can be used in the routing and accessibility function in `r5r`. The two key differences here are:
...
```{r, message = FALSE}
r5r_network_congestion <- build_custom_network(
  data_path = data_path,
...
That's it. Now we can pass the path of the `.csv` file to the `build_custom_network()` along with the original OSM `.pbf` file to build a new routable transport network with modified OSM car speeds.

```{r, message = FALSE}

r5r_network_new_speeds <- build_custom_network(
  data_path = data_path,
...
The `build_custom_network()` function has a parameter `default_speed`, which can be used to set the speed of all the roads not listed in the `.csv` file. By default, `default_speed = NULL` and the speeds of the unlisted roads are kept unchanged.
