...
urlcolor: blue
vignette: >
  %\VignetteIndexEntry{Modify car speeds} 
...
And that's it ! You can now use the new network `r5r_core_congestion` in any of `r5r`'s routing and accessibility functions, like this.
...
In this example, we'll set different speed factor for roads of different hierarchy levels. We can assume for example that congestion levels tend to be more intense in roads of higher hierarchy. The first thing we need to do is to determine the OSM edge ids by road type.

We can do this by reading the OSM data from our `.pbf` file in just a couple of steps.
...
Here's how this road network looks like.
...
After we add a new column `"max_speed"` with values conditioned on the road type, the `data.frame` looks like this:
...
Now we just need to save this table as a `.csv` file and pass it to the `modify_osm_carspeeds()` along with the original OSM `.pbf` file.
...
r5r_core_new_speeds <- modify_osm_carspeeds(
  pbf_path = pbf_path,
  csv_path = csv_path,
  output_dir = tempdir(),
  percentage_mode = TRUE,
...
The `modify_osm_carspeeds()` function has a parameter `default_speed`, which you can use to set the speed of all the roads not listed in the `.csv` file. By default, `default_speed = NULL` and the speeds of the unlisted roads are kept unchanged.

In this example, we'll use this parameter to apply the same speed factor to all roads. To do this, can only need to create a `.csv` file with a mock OSM id that that does not exist in our OSM data.
...
# create a temp dir
new_temp_dir <- fs::path_temp(tmpdir='./20kmph')
dir.create(new_temp_dir)

# build network with all roads at at 20 Km/h 
r5r_core_20kmph <- r5r::modify_osm_carspeeds(
  pbf_path = pbf_path,
  csv_path = mock_csv,
...
```{r, message = FALSE}
r5r::stop_r5(r5r_core_congestion)
rJava::.jgc(R.gc = TRUE)
```
