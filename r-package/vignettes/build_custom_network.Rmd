...
In summary, OSM speeds and LTS can be changed using two different strategies: (1) by applying changes to individual road segments, or (2) by applying changes only to the roads within determined polygons. Let's see how this works with a few examples using the a sample data set for the city of Porto Alegre (Brazil) included in `r5r`.
...
Now you can simply run any routing or accessibility using the function passing the new car speeds parameters `new_carspeeds` and `carspeed_scale`:
...
In this example, we'll simulate as if the speed limits of all roads were changed to 40 Km/h. To do this, we simply edit the `new_edge_speeds` table we created in our previous example.
...
can use one or more spatial polygons to set speed the new car speeds of all the 
roads within those polygons. In this example with the sample data from {r5r}, we have two polygons in the city of Porto Alegre. The first one is covers the extended city center, and the second polygon covers a few important roads that connect two major avenues in the city.
...
There are two approaches to changing LTS, pretty similar to changing road speeds. 
This can be done either with a `data.frame` indicating the new LTS of each individual OSM edge id, or an `sf data.frame` polygon that will update the LTS of all the roads that fall within each polygon.
...
head(edge_lts)
```
Now we simply need to pass this `data.frame` with the new LTS values to the parameter `new_lts` in a routing/accessibility function:
...
Alternatively, we can simulate that all roads within a given polygon received new cycling infrastructure.

```{r, message = FALSE}
# read sf with congestion polygons
lts_poly <- readRDS(file.path(data_path, "poa_mls_lts.rds"))

# preview
mapview::mapview(lts_poly, zcol="lts")
```

And we only need to pass this polygon with the new LTS values to the `new_lts` parameter in any routing/accessibility function:
