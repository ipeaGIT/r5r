...
4. `expanded_travel_time_matrix()` for calculating minute-by-minute travel time
estimates between origin destination pairs within a specified time window;
...
## Building routable transport network with `setup_r5()`
...
information.  It estimates for each origin destination pair the travel time of 
the fastest route alternative departing each minute within a specified time 
window. This examples below uses a 20-minute time window. Please note this 
