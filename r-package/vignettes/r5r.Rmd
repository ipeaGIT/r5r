...
1. `setup_r5()` to initialize an instance of `{r5r}` and build a routable
transport network;
...
obs. The package also includes a convenient function `modify_osm_carspeeds()` that allows one to build a routable network with modified OSM car speeds to account for different scenarios of traffic congestion and road closure. [See this vignette](https://ipeagit.github.io/r5r/articles/modify_osm_car_speeds.html).
