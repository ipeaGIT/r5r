...
Install the development version from GitHub (soon on CRAN) and load packages.
...
1. `setup_r5` to build a routable transport network
...
Let's have a quick look at these three functions using demonstration on sample data.
...
Before we start, we need to increase available memory available to Java. This is
because by default R allocates only 512MB of memory for Java processes, which is
not enough for large queries using r5r. To increase available memory to 2GB, for
example, we need to set the java.parameters option at the beginning of the script,
as follows:
...
downloads/updates a compiled JAR file of R5 and store it locally in the `r5r` package 
directory for future use; and (2) combines the osm.pbf and gtfs.zip data sets to
build a routable network object. The function 
...
For fast routing analysis, **r5** has two core functions: `travel_time_matrix` 
...
Interactive visualization of results with the `mapview` package.

```{r, message = FALSE}
mapviewOptions(platform = 'leafgl')
mapview(df, zcol = 'mode')

```

Static visualization of results with the `ggplot2` package.

To facilitate the visualization of the results in `ggplot2`, you can also use the `street_network_to_sf` that will extract the street networked passed as an input.
