...
Both of the functions we just saw (`travel_time_matrix()` and `expanded_travel_time_matrix()`) consider a departure time set by the user. In some cases, though, you might need to calculate the travel times considering an arrival time. For such cases, you can use `arrival_travel_time_matrix()` Note that the output of this function includes more information with additional columns, like in the `expanded_travel_time_matrix()` function (you can also use `breakdown = TRUE`).
