...
tester <- function(n_threads) set_n_threads(r5r_core@jcore, n_threads)
...
test_that("number of threads is set correctly", {
  ...
  expect_true(r5r_core@jcore$getNumberOfThreads() == 1)

  expect_true(tester(Inf))
  expect_true(r5r_core@jcore$getNumberOfThreads() == parallel::detectCores() - 1)
})
