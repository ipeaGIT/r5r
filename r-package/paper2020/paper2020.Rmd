...
departure times over a time window. This function uses an R5-specific extension to the RAPTOR routing algorithm developed by Conway et al. [@conway2017evidencebased], which provides an efficient and systematic sample of 
multiple departures per minute over a `time_window` set by the user. If the user
passes a `time_window` parameter to the function, r5r will calculate multiple travel 
time matrices departing each minute and return additional information on the percentile distribution of travel times that allows one to grasp service reliability issues.
...
distance of each segment of the trip. This function uses an R5-specific extension
to the McRAPTOR routing algorithm to find paths that are optimal or less than
optimal. The specific extension to McRAPTOR to do suboptimal path routing are not documented yet, but a detailed description of base McRAPTOR can be found in @delling2015roundbased.
