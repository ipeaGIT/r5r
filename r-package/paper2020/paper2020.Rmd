...
The first step when using r5r is to set the amount of memory available to Java. By default `R` allocates only 512 MB of memory to the Java Virtual Machine (JVM), which is not enough for the tasks r5r is designed to carry out, such as calculating travel time matrices on large multimodal networks. To increase available memory to 2GB[^1], for example, the user needs to set the `java.parameters` option at the beginning of the script as follows:

[^1]: Note that large study areas may require significantly more memory to run properly.
