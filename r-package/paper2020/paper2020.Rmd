...
departure times over a time window. The origin/destination input can be either a 
spatial `sf POINT` object, or a `data.frame` containing the columns `id`, `lon`, 
`lat.` The output is a `data.frame` with the travel time estimate between each 
origin/destination pair. If the user passes a `time_window` parameter to the,
function, r5r will calculate multiple travel time matrices departing each minute
and return additional information on the percentile distribution of travel times
...
#> I think this is perhaps unecessary detail for the paper publication.
...
`travel_time_matrix()` function. The function takes travel parameters and
...
reached with the given parameters:
