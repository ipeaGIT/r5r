...
departure times over a time window. The output comprises the travel time between
each origin/destination pair. If the user passes a `time_window` parameter
to the function, r5r will calculate multiple travel time matrices departing each 
minute and return additional information on the percentile distribution of travel times
that allows one to grasp service reliability issues.

- `detailed_itineraries()`: used to compute detailed information on either the
...
distance of each segment of the trip.
...
the city of Porto Alegre (Brazil). It includes four files:
...
# removes files other than the four specified above
...
files_to_remove <- all_files[! all_files %chin% desired_files]

file.remove(paste0(data_path, "/", files_to_remove))
```
...
points <- fread(system.file("extdata/poa/poa_hexgrid.csv", package = "r5r"))
...
line pointing to `setup_r5()` where the input data is stored:
...
Multiple GTFS can be used simultaneously, in which case all files are merged into
a single one automatically by R<sup>5</sup>.
