...
Note that the leftmost origin/destination pairs do not exhibit any variation in the distribution of their travel time along the time window. This is because the trips between such pairs rely solely on walking to be completed, therefore are not susceptible to transit reliability issues.
...
The picture above can be used to compare the walking-only to the walking and transit itineraries between each origin/destination pair. Given that Porto Alegre's sample `GTFS.zip` file includes only very few public transport routes, the resulting plot picture some odd scenarios, such as a fairly lengthy circular-like bus route, in the leftmost facet, and a very long walking access trip to the bus, in the rightmost facet.
