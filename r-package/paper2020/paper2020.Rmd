...
To account for variations on service levels accross the day [@stepniak2019impact], the user can use the `time_window` parameter. In this case, several matrices are calculated using one departure time every minute within the set time window. By default the function outputs the 50th percentile (i.e. the median) travel time between each origin and destination pair  within that time window. The `percentiles` parameter allows the user to retrieve travel time estimates at different points of the distribution. An example of the `travel_time_matrix()` function's usage is presented below:
