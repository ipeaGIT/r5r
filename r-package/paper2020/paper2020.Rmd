...
deterministic estimates of travel times without accounting for variability in 
results (due to service reliability issues, for example) and only return the 
fastest route option between a set of origins and destinations (see @conway2018accounting 
for a more in-depth discussion). This simple approach overlooks how often people's 
...
documented yet, but a detailed description of base McRAPTOR can be found in @delling2015roundbased.
...
By default `R` allocates only 512 MB of memory for Java processes. To increase 
available memory to 2GB, for example, we need to set the `java.parameters` 
option at the beginning of the script as follows:
...
- The spatial coordinates of some of the city's points of interest.
...
The image below depicts the travel time distribution for each origin/destination in which a trip could be completed within the specified parameters. The pairs are ordered in the x axis by their median travel time and each point in the plot area pictures a distinct distribution percentile.
...
plot_data[, travel_time := fifelse(travel_time > max_trip_duration, NA_integer_, travel_time)]
plot_data[, id := factor(paste0(fromId, "_", toId), levels = id_order)]

ggplot(plot_data) + 
   geom_point(aes(id, travel_time), alpha = 0.02, size = 1) + 
   theme_minimal() +
