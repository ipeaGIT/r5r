...
public class R5TravelTimeComputer extends TravelTimeComputer {
    ...
    public OneOriginResult computeTravelTimes() {
        ...
        for (StreetMode accessMode : accessModes) {
            ...
            if (request.hasTransit()) {
                ...
                updateStreetTimesAndModes(bestAccessOptions, travelTimesToStopsSeconds, accessMode);
//                bestAccessOptions.update(travelTimesToStopsSeconds, accessMode);
        ...
        if (request.accessModes.contains(LegMode.CAR_PARK)) {
            ...
            if (sr == null) {
                ...
                updateStreetTimesAndModes(bestAccessOptions, sr.getReachedStops(), StreetMode.CAR);
...
//            bestAccessOptions.getTimes()
            Map<LegMode, TIntIntMap> accessTimes = new HashMap<>();
            ...
            McRaptorSuboptimalPathProfileRouter mcRaptorWorker = new McRaptorSuboptimalPathProfileRouter(network,
                    request, accessTimes, null, listSupplier, InRoutingFareCalculator.getCollator(request));
    ...
    void updateStreetTimesAndModes(StreetTimesAndModes options, TIntIntMap times, StreetMode streetMode) {
        times.forEachEntry((stop, time) -> {
            if (!options.streetTimesAndModes.containsKey(stop) || time < options.streetTimesAndModes.get(stop).time) {
                options.streetTimesAndModes.put(stop, new StreetTimesAndModes.StreetTimeAndMode(time, streetMode));
            }
            return true;
        });
    }
